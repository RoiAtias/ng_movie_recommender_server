...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = hidden.clamp(min=0)  # Shape: (N, H)
...
def nn_forward_backward(
    ...
    ############################################################################
    # Compute the loss
    # Numeric stability trick: subtract max score from each score
    ...
    grads['W2'] = hidden.t().mm(dscores) + 2 * reg * W2
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        for param_name, param in params.items():
            params[param_name] = param - learning_rate * grads[param_name]
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_get_search_params():
    ...
    learning_rate_decays = [1.0, 0.95, 0.9, 0.80, 0.70]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    # Create a new instance of TwoLayerNet
                    net = TwoLayerNet(input_size=X_train.shape[1], hidden_size=hidden_size,
                    ...
                    # Print the hyperparameters and validation accuracy
                    print(f'lr: {lr}, hidden_size: {hidden_size}, reg: {reg}, lr_decay: {lr_decay}, val_acc: {val_acc}')
